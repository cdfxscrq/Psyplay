...
<?php } else { ?>
<img id="logo-footer" border="0" src="<?php echo get_template_directory_uri(); ?>/assets/css/img/logo.png" alt="<?php bloginfo('name') ?>" class="mv-ft-logo" />
<?php } ?>
</p>
...
<?php if($copy = get_option('disclaimer_footer')) { ?>
					...
					<?php } else { ?>
					<p style="font-size: 11px; line-height: 14px; color: rgba(255,255,255,0.4)"><?php _e('Disclaimer: This site does not store any files on its server. All contents are provided by non-affiliated third parties.', 'psythemes'); ?></p>
						<?php } ?>
                </div>
                <div class="clearfix"></div>
            </div>
...
<a href="<?php echo get_permalink(); ?>#mv-info" class="btn bp-btn-light" <?php if (is_single() && is_post_type('tvshows')){?>style="display:none;"<?php }?>><i class="fa fa-lightbulb-o" ></i> <span><?php _e('Turn off light', 'psythemes');?></span></a>
<?php }?>
...
<?php if($pagina = series_get_meta('ep_pagination')) { 
	get_template_part('includes/single/parts/series/pagination/'. $pagina .'');
		} else { 
	get_template_part('includes/single/parts/series/pagination/default'); 
	} 
?>
<?php }?>
<?php }?>

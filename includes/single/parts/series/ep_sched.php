...
$tz = series_get_meta('timezone');
$value =  array($dato, $tz, $cd);
$message = $note;
 echo str_replace($tags, $value, $message); 
 } else {
$message = __('Estimated the next episode will come at');
echo $message.$dato.' '.$tz.'<span id="date"></span>';
 }?>
</div>
</div>
<?php }?>